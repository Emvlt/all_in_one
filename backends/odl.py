...
class ODLBackend:
    ...
    def get_filtered_backprojection(
        self, sinogram: np.ndarray, filter_name: str
    ) -> np.ndarray:
        assert (
            self.initialised
        ), "ODL backend not initialised, consider running odl_backend.initialise_odl_backend_from_metadata_dict"
        fbp = odl.tomo.fbp_op(
            self.operator, filter_type=filter_name, frequency_scaling=0.8
        )
        return fbp(sinogram)
