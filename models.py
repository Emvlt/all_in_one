...
class FourierTransformInceptionLayer(nn.Module):
    def __init__(self, dimension:int, n_channels_input:int, n_channels_output:int, n_filters:int, dtype:torch.dtype, device:torch.device) -> None:
        super(FourierTransformInceptionLayer, self).__init__()
        if dimension == 1:
            self.conv1 = nn.Conv1d(n_channels_input, n_filters, 1, padding = 0, dtype=dtype, device=device)
            self.conv3 = nn.Conv1d(n_channels_input, n_filters, 3, padding = 1, dtype=dtype, device=device)
            self.conv5 = nn.Conv1d(n_channels_input, n_filters, 5, padding = 2, dtype=dtype, device=device)
            self.conv7 = nn.Conv1d(n_channels_input, n_filters, 7, padding = 3, dtype=dtype, device=device)
            self.collection_filter = nn.Conv1d(4*n_filters, n_channels_output, 7, padding = 3, dtype=dtype, device=device)

        elif dimension == 2:
            self.conv1 = nn.Conv2d(n_channels_input, n_filters, (1,1), padding = (0,0), dtype=dtype, device=device)
            self.conv3 = nn.Conv2d(n_channels_input, n_filters, (1,3), padding = (0,1), dtype=dtype, device=device)
            self.conv5 = nn.Conv2d(n_channels_input, n_filters, (1,5), padding = (0,2), dtype=dtype, device=device)
            self.conv7 = nn.Conv2d(n_channels_input, n_filters, (1,7), padding = (0,3), dtype=dtype, device=device)
            self.collection_filter = nn.Conv2d(4*n_filters, n_channels_output, 7, padding = 3, dtype=dtype, device=device)
...
class AverageModule(nn.Module):
    def __init__(self, n_measurements:int, n_filters:int, device:torch.device):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels = n_measurements, out_channels = n_measurements, kernel_size  = (n_filters,1), padding=(0,0),  dtype=torch.cfloat, device=device)
        self.conv2 = torch.nn.Conv2d(in_channels = n_measurements, out_channels = n_measurements, kernel_size  = (n_filters,3), padding=(0,1),  dtype=torch.cfloat, device=device)
        self.conv3 = torch.nn.Conv2d(in_channels = n_measurements, out_channels = n_measurements, kernel_size  = (n_filters,5), padding=(0,2),  dtype=torch.cfloat, device=device)
        self.conv4 = torch.nn.Conv2d(in_channels = n_measurements, out_channels = n_measurements, kernel_size  = (n_filters,7), padding=(0,3),  dtype=torch.cfloat, device=device)
...
class UpModule(nn.Module):
    def __init__(self, dimension:int, n_channels_input:int, n_channels_output:int, n_filters:int):
        super().__init__()
        if dimension == 1:
            self.up = nn.ConvTranspose1d(n_channels_input, n_channels_input, 4, stride = 2, padding = 1)
            ...
            self.up = nn.ConvTranspose2d(n_channels_input, n_channels_input, 4, stride = 2, padding = 1)
...
class Iteration(nn.Module):
    ...
    def dual_operation(self, primal:torch.Tensor, dual:torch.Tensor, input_sinogram:torch.Tensor) -> torch.Tensor:
        if self.dimension == 1:
            ...
            x += dual
            '''
            return (dual + self.dual_block( torch.cat([dual, self.op(primal[:, 0:1, ...]).squeeze(dim=1) / self.operator_norm, input_sinogram / self.operator_norm ], dim=1)))
        elif self.dimension == 2:
            return dual + self.dual_block( torch.cat([dual, self.op(primal[:, 0:1, ...]) / self.operator_norm, input_sinogram / self.operator_norm ], dim=1))
        else:
            raise ValueError
    ...
    def primal_operation(self, primal:torch.Tensor, dual:torch.Tensor) -> torch.Tensor:
        if self.dimension == 1:
            ...
            x += primal
            '''
            return primal + self.primal_block(torch.cat([primal, self.op_adj(dual[:,:self.n_measurements].unsqueeze(dim=1)) / self.operator_norm], dim=1))
        elif self.dimension == 2:
            return primal + self.primal_block(torch.cat([primal, self.op_adj(dual[:, 0:1, ...]) / self.operator_norm], dim=1))
        else:
            raise ValueError

    def forward(self, primal:torch.Tensor, dual:torch.Tensor, input_sinogram:torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        # dual block
...
class LearnedPrimalDual(nn.Module):
    ...
    def forward(self, input_sinogram:torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
