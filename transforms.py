...
class FieldsCorrect(object):
    def __init__(self, device:torch.device, sinogram_size:torch.Size, flat_field=None, dark_field=None) -> None:
        if dark_field is None:
            self.dark_field = torch.zeros(sinogram_size).to(device)
        else:
            self.dark_field = dark_field
        if flat_field is None:
            self.flat_field = torch.ones(sinogram_size).to(device)
        else:
...
class PoissonSinogramTransform(object):
    def __init__(self, I0:int, device:torch.device, sinogram_size:torch.Size, flat_field=None, dark_field=None):
        self.sinogram_transform = Compose([
            Exponentiate(I0),
            FieldsUncorrect(device, sinogram_size, flat_field, dark_field),
            PoissonNoise(),
            FieldsCorrect(device, sinogram_size, flat_field, dark_field),
            Logarithm(I0)
        ])
...
class CompleteSinogramTransform(object):
    def __init__(self, I0:int, device:torch.device, sinogram_size:torch.Size, sigma:int, cross_talk:float, flat_field=None, dark_field=None):
        self.sinogram_transform = Compose([
            Exponentiate(I0),
            FieldsUncorrect(device, sinogram_size, flat_field, dark_field),
            PoissonNoise(),
